...
# Variables for paddle resizing
rec1_resize_start = None
rec2_resize_start = None
resize_duration = 5000  # 5 seconds
...
def reset_game():
    global game_state, scores, ball_pos, ball_vel, rec1, rec2, scored
    global pu_pos, pu2_pos, pu_vel, pu2_vel
    global rec1_resize_start, rec2_resize_start
    global rand_operation, rand_operation2, throw_rand, throw_rand2
...
while running:
    ...
    if game_state == "menu":
        ...
        if ball.colliderect(rec1) or ball.colliderect(rec2):
            ball_vel[0] = abs(ball_vel[0]) if ball.colliderect(rec1) else -abs(ball_vel[0])
        ...
        if scored:
            ball_pos[:] = [400, 300]
            ball_vel[:] = [speed_slider.getValue(), random.choice([-3, 3])]
            rec1.y = 250
            rec2.y = 250
            scored = False
